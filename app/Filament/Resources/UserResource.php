            ...
            ->toolbarActions([
                BulkActionGroup::make([
                    BulkAction::make('bulk_print')
                        ...
                        ->color('gray')
                        ->url(route('users.print-bulk'))
                        ->livewireClickHandlerEnabled(false)
                        ->accessSelectedRecords(false)
                        ->openUrlInNewTab(true)
                        ->extraAttributes([
                            'style' => 'padding: 10px 16px !important;',
                            'x-bind:href' => "`\${window.location.origin}/print/users-bulk?ids=\${[...selectedRecords].join(',')}`",
                        ]),
                    BulkAction::make('bulk_delete')
                        ...
                        ->hidden(fn(): bool => ! static::canDeleteAny())
                        ->modalHeading('Hapus User Terpilih')
                        ->modalDescription(fn(): string => static::hasDeletePasswordVerification()
                            ? 'Semua user terpilih akan dihapus permanen.'
                            : 'Semua user terpilih akan dihapus permanen. Verifikasi password hanya diminta sekali per sesi login.')
                        ->form(fn(): array => static::hasDeletePasswordVerification()
                            ? []
                            : [
                                TextInput::make('password')
                        ...
                        ->action(function (array $data, $records): void {
                            ...
                            if ($deleted > 0) {
                                Notification::make()
                                    ->success()
                                    ->title('User berhasil dihapus')
                                    ->body("{$deleted} user berhasil dihapus.")
                                    ->send();
                            }
                            ...
                            if ($skipped > 0) {
                                Notification::make()
                                    ...
                                    ->body("{$skipped} user dilewati karena masih dibutuhkan oleh sistem.")
                                    ->send();
                            }
                        })
