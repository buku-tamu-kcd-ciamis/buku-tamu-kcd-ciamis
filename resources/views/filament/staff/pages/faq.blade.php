<x-filament-panels::page>
    <div class="w-full space-y-6 pb-8" x-data="{ openFaq: null, searchQuery: '' }">
        ...
        <div class="relative">
            ...
            <input type="text" x-model="searchQuery" placeholder="Cari pertanyaan..."
                class="w-full pl-12 pr-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-primary-400 dark:focus:border-primary-500 focus:ring-2 focus:ring-primary-200 dark:focus:ring-primary-800 transition-all duration-200" />
        </div>

        {{-- FAQ Items --}}
        <div class="space-y-4">
            @forelse($this->getFaqs() as $index => $faq)
                <div x-show="searchQuery === '' || '{{ strtolower($faq['question']) }}'.includes(searchQuery.toLowerCase()) || '{{ strtolower(strip_tags($faq['answer'])) }}'.includes(searchQuery.toLowerCase())"
                    ...
                    <div class="relative rounded-2xl border-2 bg-white dark:bg-gray-800 transition-all duration-300 overflow-hidden"
                        ...
                        <div class="absolute inset-0 bg-linear-to-r from-primary-50/50 to-transparent dark:from-primary-900/20 opacity-0 transition-opacity duration-300"
                            :class="{ 'opacity-100': openFaq === {{ $index }} }"></div>
                        ...
                        <button @click="openFaq = (openFaq === {{ $index }} ? null : {{ $index }})"
                            ...
                            <div class="flex items-center gap-4 flex-1">
                                ...
                                <div class="flex-1">
                                    <span
                                        class="font-semibold text-gray-900 dark:text-white text-base leading-relaxed block"
                        ...
                        <div x-show="openFaq === {{ $index }}" x-collapse
                            ...
                            <div class="relative px-6 pb-6 pt-2">
                                <div class="pl-14 pr-4">
                                    <div
                                        class="bg-linear-to-br from-gray-50 to-white dark:from-gray-700/50 dark:to-gray-800/50 rounded-2xl p-6 border-l-4 border-primary-500 shadow-inner">
                                        <div
                                            class="text-gray-700 dark:text-gray-200 leading-relaxed space-y-3 prose prose-sm dark:prose-invert max-w-none">
                                            {!! $faq['answer'] !!}
                                        </div>
            ...
            @empty
                <div class="text-center py-12">
                    <x-heroicon-o-question-mark-circle class="w-16 h-16 mx-auto text-gray-300 dark:text-gray-600 mb-4" />
                    <p class="text-gray-500 dark:text-gray-400 text-lg">Belum ada FAQ yang tersedia.</p>
                    <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">FAQ akan ditambahkan oleh administrator.</p>
                </div>
            @endforelse
        </div>
